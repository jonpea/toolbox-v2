...
%     function c = temp(id, xyglobal)
%         [local.x, local.y] = elmat.cols( ...
%             contract(source.Frame(id, :, :), xyglobal, 2));
%         local.azimuth = cart2pol(local.x, local.y);
%         c = source.Pattern(local.azimuth);
%     end
% source.Gain = antennae.multipattern(@temp);

source.Gain = antennae.multipattern( ...
    antennae.localfunction(source.Pattern, source.Frame, @specfun.cart2pol, 1));
...
if options.Plotting
    ...
    x = cos(azimuth);
    y = sin(azimuth);
    radius = source.Gain(ones(size(x)), [x(:), y(:)]);
