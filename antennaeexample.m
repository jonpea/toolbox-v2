...
import elmat.nrows
    ...
    function labelaxis(ax, origins, frames, direction, local)
        ...
        points.text(ax, ...
            ...
            compose(local.format, index(origins, 1), direction), ...
            'Interpreter', 'latex', ...
            'FontSize', fontsize);
    end
    ...
    function plotaxes(ax, origins, frames)
        ...
        points.text(ax, ...
            origins, ...
            compose(graphics.untex('$\mathbf{o}_%d$'), index(origins, 1)), ...
            'Interpreter', 'latex', ...
            'FontSize', fontsize)
...
import power.framefunctionnew
import power.isofunction
import power.polarpattern
pattern1 = @(phi) phi/(2*pi);
pattern2 = @(phi) 1 - phi/(2*pi);
unitcircle = isofunction(1.0);
gainpattern = framefunctionnew({ ...
    polarpattern(pattern1) ...
    polarpattern(pattern2) ...
    }, frames, ...
    [1, 2]);
