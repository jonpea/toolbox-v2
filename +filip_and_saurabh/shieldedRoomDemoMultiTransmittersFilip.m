function shieldedRoomDemoMultiTransmittersFilip(varargin)
...
source.Origin = [ % [m]
    inplanepoint(xquantile, yquantile); % access point #1
    inplanepoint(0.78, 0.33);             % access point #2
  %  inplanepoint(0.8, 0.8);             % access point #3
    ];
source.Gain = [ % [dBW]
    1.0d0; % access point #1
    1.0d0; % access point #2
 %   1.0d0; % access point #3
    ];
source.Frequency = [ % [Hz]
    2.4d9; % access point #1
    2.4d9; % access point #2
%    2.4d9; % access point #3
    ];
source.Channel = [
    1;
    1;
...
disp(size(downlinks.SINRatio));
sinr = reshape(downlinks.SINRatio, size(gridx));
surf(ax, gridx, gridy, sinr, ...
    'EdgeAlpha', 0.0', 'FaceAlpha', 0.9)
contour(ax, gridx, gridy, sinr, 10, 'Color', 'white', 'LineWidth', 1)
title(ax, 'SINR')
patch(ax, ...
    'Faces', fv3Dold.Faces(1 : end - 2, :), ...
    'Vertices', fv3D.Vertices, ...
