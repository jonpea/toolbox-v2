function filip_new_room_3(varargin)
...
parser.addParameter('NumSamplesX', 20, @isscalar)
parser.addParameter('NumSamplesY', 40, @isscalar)
parser.addParameter('QuantileX', 0.1, @isscalar)
parser.addParameter('QuantileY', 0.9, @isscalar)
parser.addParameter('QuantileZ', 0.99, @isscalar)
parser.addParameter('Delta', 1e-3, @isscalar)
parser.addParameter('GainThreshold', -20, @isscalar)
parser.addParameter('ConcreteIndices', 5) %I added a 0 here so there is no concrete walls anywhere
...
sourcegain = antennae.dispatch( ...
    @sourcePattern, ...
    1, ... % "maps Antenna #1 to Gain Pattern #1"
    antennae.orthocontext(sourceFrame, @specfun.cart2usphi)); % "Cartesian to spherical on unit sphere"
