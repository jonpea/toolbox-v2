function filip_new_room(varargin)
%% Calculations from Yuen's IEEE TAP paper.
% look at the rays. try to have a non complex walls for reflection and
% transmission
% Focus on seeing if isotropic works make it symetrical.
%%
parser = inputParser;
parser.addParameter('Arities', 0:2, @isvector)
parser.addParameter('Reporting', false, @islogical)
parser.addParameter('NumSamplesX', 100, @isscalar)
parser.addParameter('NumSamplesY', 100, @isscalar)
parser.addParameter('QuantileX', 0.1, @isscalar)
parser.addParameter('QuantileY', 0.9, @isscalar)
parser.addParameter('QuantileZ', (3.15/3.2), @isscalar)
...
gibIndices(end + (1 : 2)) = (size(fv2D.Faces,1)+1):(size(fv2D.Faces,1)+2);
...
source.Origin = [ % [m]
    inplanepoint(0.5,0.5); % access point #1
%     inplanepoint(0.8,0.3); % access point #2
    ];
...
% sourcePatternBasic = antennae.isopattern(0.0);
...
axis1 = matfun.unit([0  0  -1]);
axis2 = matfun.unit([0  1  0]);
axis3 = specfun.cross(axis1, axis2);

% SourceFrameBasic = cat(3, ...
%     [1  1  0], ... % local x-axis
%     [1 -1  0], ... % local y-axis
%     [0  0  1]);    % local z-axis
SourceFrameBasictwo = cat(3, ...
    axis1, ... % local x-axis
    axis2, ... % local y-axis
    axis3);    % local z-axis
% phi = deg2rad(0); % [rad]
...
origin1 = [1 1 1]; % "local origin"

[phi, theta] = sx.meshgrid( ...
    linspace(0.05, 2*pi, 40), ...
    linspace(delta, pi - delta, 40)); % avoid singularity at "0 == pi"
[u1, v1, w1] = specfun.sphi2cart(phi, theta, sourcePattern(phi, theta));
...
radius = sourcePattern(azimuth, inclination);
...
multigain = antennae.dispatch( ...
    {@sourcePattern}, ...
    1, ... % trivial mapping as each entity has its own pattern
    context);

radii = multigain(indices, directions1);
xyznew = origins(indices, :) + radii.*matfun.unit(directions1, 2);
points.plot(ax, xyznew, 'ro')
%%

% test = [x1,y1,z1;x2,y2,z2;x3,y3,z3];
% disp(test*test')
sourcegain = antennae.dispatch( ...
    @sourcePattern, ...
    [1], ... % "maps Antenna #1 to Gain Pattern #1 and Antenna #2 to Gain Pattern #1"
    antennae.orthocontext( ...
    cat(1, SourceFrameBasictwo), ...
...
%Possible text files to use
%gib_eng_tower_TE_refl_1.8GHz.txt
%Wall1_TE_refl_1GHz.txt
%constant_reflection.txt
reflectiongains = antennae.dispatch({
    makepattern('Wall1_TE_refl_1GHz.txt') ... % gib/reflection
...
%Possible text files to use
%gib_eng_tower_TE_trans_1.8GHz.txt
%Wall1_TE_trans_1GHz.txt
%constant_transmission.txt
transmissiongains = antennae.dispatch({
    makepattern('Wall1_TE_trans_1GHz.txt') ... % gib/transmission
...
surf(ax, gridx, gridy, sinr, ...
    'EdgeAlpha', 0.0', 'FaceAlpha', 0.9)
title(ax, 'SINR plot of a 16mx12m room with two Access Points')
patch(ax, ...
    'Faces', fv3Dold.Faces(1 : end - 2, :), ...
    'Vertices', fv3D.Vertices, ...
    'FaceAlpha', 0.05, ...
