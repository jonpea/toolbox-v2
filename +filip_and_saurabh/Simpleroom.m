...
[linevertices, materialsdata, wallmaterials] = filip_and_saurabh.five;
...
aporigin1 = [7.5, 7.5, 1.5]; % [m]
aporigin3=[7.5,7.5,4.5];
aporigin4 = [7.5,7.5, 7.5];
% aporigin5 = [5, 3, 4.5]; % [m]
% aporigin6=[12,3,4.5];
% aporigin7 = [22,3, 4.5];
% aporigin8 = [5, 3, 7.5]; % [m]
...
source.Position = [
    aporigin1;
    aporigin3; % same as "cat(1, aporigin1, aporigin2)"
    aporigin4;
%     aporigin4;
%     aporigin5;
%     aporigin6;
...
% frame9=frame1;
% frame10=frame1;
source.Frame = cat(1, frame1,frame2,frame3); %joining 2 frames along 1st dimension(rows)
...
source.Gain = antennae.dispatch( ...
    ...
    source.Pattern source.Pattern}, [1 1 1], ...
...
figure(10), clf
powersumOverAntennae_db = specfun.todb(powersumOverAntennae);
for i = 1 : size(gridx, 3) % number of slices in z
    subplot(1, size(gridx, 3), i)
    surfc(gridx(:,:,i), gridy(:,:,i), powersumOverAntennae_db(:, :, i), 'EdgeAlpha', 0.1)
    ...
    view(-35, 25)    
    caxis([min(powersumOverAntennae_db(:)), max(powersumOverAntennae_db(:))])
end
