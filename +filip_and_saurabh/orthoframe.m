function filip_new_room_patch(varargin)
...
parser.addParameter('NumSamplesY', 120, @isscalar)
parser.addParameter('QuantileX', 0.1, @isscalar)
parser.addParameter('QuantileY', 0.9, @isscalar)
parser.addParameter('QuantileZ', (3.2/3.3), @isscalar)
...
source.Origin = source.Origin + 1e-7*randn(size(source.Origin));
...
sourcePatternBasic = antennae.isopattern(0.0);
    ...
    function result = sourcePattern(phi, theta)
       ...
       thetaThreshold = 0.5*pi;
        scaling = 0.00000000000000000001;
        c = 3d8;
        f = 2.45d9;
        Lambda = c/f;
        ...
        %spy(repmat(mask, 1, 1000))
        %result(1:floor((size(result)/2))) = scaling.* result(1:floor((size(result)/2)));
    end
...
reflectiongains = antennae.dispatch({
    makepattern('gib_eng_tower_TE_refl_1.8GHz.txt') ... % gib/reflection
...
transmissiongains = antennae.dispatch({
    makepattern('gib_eng_tower_TE_trans_1.8GHz.txt') ... % gib/transmission
