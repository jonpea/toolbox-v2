function filip_new_room_180904(intensity, varargin)
...
parser.addParameter('NumSamplesX', 400, @isscalar)
...
source.Origin = [ % [m]
    inplanepoint(0.6,0.6); % access point #1
    inplanepoint(0.4,0.4); % access point #2
    ];
source.Origin = delta + source.Origin;
% source.Gain = [ % [dBW]
%     (220/1); % access point #1
%     (220/1); % access point #2
%     
%     ];
source.Frequency = [ % [Hz]
...
%sourcegain = antennae.isopattern(0.0);
    ...
    function result = sourcepattern(phi, theta)
        ...
        result = 5.0*(sin(sinsin)./sinsin) .* cos(sincos);
...
%     @sourcepattern, ...
%     [1,1], ... % "maps Antenna #1 to Gain Pattern #1 and Antenna #2 to Gain Pattern #1"
%     antennae.orthocontext( ...
%     cat(1, SourceFrame_north_east, SourceFrame_south_west), ...
%     @specfun.cart2usphi)); % "Cartesian to spherical on unit sphere"

sourcegain = antennae.isopattern(intensity);
% <===== New code =====
...
reflectiongains = antennae.dispatch({
    makepattern('constant_reflection.txt') ... % gib/reflection
...
transmissiongains = antennae.dispatch({
    makepattern('constant_transmission.txt') ... % gib/transmission
...
surf(ax, gridx, gridy, sinr, ...
    'EdgeAlpha', 0.0', 'FaceAlpha', 0.99)
caxis(ax, [0, 20])
%title(ax, 'SINR plot of a 16m x 10m room with two Access Points')
patch(ax, ...
    'Faces', fv3Dold.Faces(1 : end - 2, :), ...
    'Vertices', fv3D.Vertices, ...
    'FaceAlpha', 0.15, ...
...
%     'EdgeAlpha', 0.93, ...
%     'FaceColor', 'red');
contour(ax, gridx, gridy, sinr, 14, 'Color', 'black', 'LineWidth', 2)
view(ax, 2)
axis(ax, 'equal', 'off', 'tight')
rotate3d(ax, 'on')
colormap(ax, jet)
...
ylabel(hBar1,'SINR(dB)','FontSize',12);
