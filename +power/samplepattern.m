function samplepattern(gain, origin, varargin)
...
[x, y, z] = sphericaltocartesian(azimuth, inclination);
    function result = sample(origin, frame, local)
        ...
        result.XDirections = reshape(local.xyz(:, 1), shape);
        result.YDirections = reshape(local.xyz(:, 2), shape);
        result.ZDirections = reshape(local.xyz(:, 3), shape);        
