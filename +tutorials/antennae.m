...
% computational efficiency, the function is called only one; its first
...
% conforms to the interface of the following example:
    function dBW = isotropicPattern(index, directions) %#ok<DEFNU>
        assert(all(ismember(index, 1 : 2)))
        assert(numel(index) == size(directions, 1))
        dBW = zeros(size(index));
        dBW(index == 1) = -10; % "dry walls"
        dBW(index == 2) = -50; % "concrete walls"
        arguments.unused(directions)
    end
