classdef Interactive < matlab.mixin.Copyable
    ...
    properties (Hidden = true, SetAccess = private)
        ...
        % Interactive state
        EditMode = interactive.EditModes.None % AccessPoint | Mobile | None
        EditIndex = 0
        EditMotion = false
    ...
    methods (Access = public)
        ...
        function obj = Interactive(faces, vertices, accesspoints, mobiles, varargin)
            ...
            parser.addParameter('ContourLabels', 'on', @(s) ismember(s, {'on', 'off'}))
            parser.addParameter('ContourColorMap', binarycolormap, @(m) ismatrix(m) && size(m, 2) == 3)
            ...
            parser.addParameter('Wavelength', elmat.lightspeed/centerfrequency, @(lambda) isscalar(lambda) && 0 < lambda)
            ...
            if isempty(options.VerifyCalculations)
                ...
                options.VerifyCalculations = ...
                    ...
                    isnumeric(options.AccessPointGains);
            end
            assert( ...
                ~options.VerifyCalculations || ...
                isequal(options.ReflectionArity, 0), ...
            ...
            obj.SINRThreshold = options.SINRThreshold;
            obj.SPMD = options.SPMD;
            ...
            if onofftological(options.ContourMap)
                ...
                % Set levels on contour plot: Contours to come later
                [obj.ContourMatrix, obj.ContourHandle] = ...
                    obj.paint(@contourf, ...
                    obj.ContourGridX, ...
                    obj.ContourGridY, ...
                    zeros(size(obj.ContourGridX)), ... 
                    [-realmax, obj.SINRThreshold]);                 
                obj.paint(@colormap, obj.ContourColorMap)
                uistack(obj.ContourHandle, 'bottom');                
                if onofftological(options.ContourLabels)
                    clabel(obj.ContourMatrix, obj.ContourHandle);
                end
            ...
            obj.Scene = scenes.Scene(faces, vertices);
            assert(size(obj.WallTable, 1) == size(obj.Faces, 1))
            
            % Gain functions
            obj.FreeGain = antennae.friisfunction(options.SignalFrequency);
            obj.SinkGain = antennae.isopattern(options.MobileGains);
            obj.SourceGain = antennae.isopattern(options.AccessPointGains);
            obj.TransmissionGain = antennae.isopattern(options.TransmissionGains);
    ...
    % Public methods for refreshing the graphical display
    methods (Access = public)
    ...
    % Private methods for refreshing the graphical display
    methods (Access = private)
        ...
        function refreshDisplayAccessPoints(obj)
            ...
            currentmode = obj.EditMode;
            currentidentifier = obj.EditIndex;
        ...
        function refreshDisplayWalls(obj)
            ...
            function refreshdisplay(wall)
                ...
                set(wall.LineHandle, ...
                    'LineWidth', width, 'Color', graphics.rgb.gray(0.6))
            end
        ...
        function refreshDisplayLinks(obj)
            ...
            switch obj.EditMode
                ...
                case interactive.EditModes.Mobile
                    mobile = obj.MobileList(obj.EditIndex);
        ...
        function refreshDisplay(obj, updatecontours)
            ...
            switch obj.EditMode
                case interactive.EditModes.None
                    return
                case interactive.EditModes.AccessPoint
                    currentlist = obj.AccessPointList;
                case interactive.EditModes.Mobile
                    currentlist = obj.MobileList;
                otherwise
                    assert(false, illegalstatemessage)
            end            
            entity = currentlist(obj.EditIndex);
    ...
    methods (Access = private)
        ...
        function updateLinks(obj)
            ...
            % Unfortunately, MATLAB doesn't seem to 
            % cope with "@obj.Scene.reflections" etc.
            scene = obj.Scene;
            
            [dlinks, ulinks] = rayoptics.analyze( ...
                ... @scene.reflections, ...
                ... @scene.transmissions, ...
                ... scene.NumFacets, ...
                scene, ...
            ...
            downlinkgain = specfun.fromdb(dlinks.GainDBW);
            receivedpower = transpose(specfun.todb(downlinkgain));
            ...
            if obj.VerifyCalculations
                ...
                compare(receivedpowerold, specfun.todb(downlinkgain'), 'rxpower.old vs downlinkgain')
                
                if isempty(obj.MobileList)
                    return
                end
                
                %obj.fprintf('  comparing fields')
    ...
    methods (Access = private)
        ...
        function dsinr = calculateDownlinkSINR(obj, xfield, yfield)
            ...
            links = rayoptics.analyze( ...
                ...
                'SourceGain', obj.SourceGain, ...
                'TransmissionGain', obj.TransmissionGain, ...
                'SinkGain', antennae.isopattern(0.0), ... % NB: zero on downlinks
                'Reporting', false);
            
            downlinkgain = specfun.fromdb(links.GainDBW);
            ...
            show = @(fig, numfigs, values, label) suppress({
                ...
                void(@() set(gca, 'ZScale', 'log', 'Color', graphics.rgb.gray));
                });
            ...
            if obj.VerifyCalculations
                ...
                obj.fprintf('######## Recomputing Downlink SINR ########\n')
                ...
                if ~obj.Quiet
                    show(1, 2, specfun.todb(sum(intermediate.RxPower, 3)), 'todb(sum(rxpower, 3))')
                ...
                if ~obj.Quiet
                    show(1, 1, specfun.todb(sum(rxpower, 3)), 'todb(sum(rxpower, 3))')
                end
            end
    ...
    methods (Access = private)
        ...
        function windowButtonDown(obj, handle, ~)
            ...
            obj.EditIndex = unknownvalue;
            obj.EditMode = interactive.EditModes.None;
            ...
            switch obj.EditMode
                case interactive.EditModes.None
                    assert(obj.EditIndex == unknownvalue)
                    return % no need to update display, below
                case interactive.EditModes.AccessPoint
                    obj.modifyAccessPoint(obj.EditIndex, 'Position', xcurrent, ycurrent)
                    obj.setTitle('Selected AP%d...', obj.EditIndex)
                case interactive.EditModes.Mobile
                    obj.modifyMobile(obj.EditIndex, 'Position', xcurrent, ycurrent)
                    obj.setTitle('Selected MOB%d...', obj.EditIndex)
                otherwise
                    assert(false, illegalstatemessage)
            end
            obj.updateLinks
        ...
        function windowButtonUp(obj, handle, ~)
            ...
            if isequal(obj.EditMode, interactive.EditModes.None)
                return
            end
            
            % NB: Before updating edit mode
            refreshcontours = obj.EditMotion && ...
                obj.EditMode == interactive.EditModes.AccessPoint;
        ...
        function windowButtonMotion(obj, ~, ~)
            ...
            switch obj.EditMode
                ...
                case interactive.EditModes.AccessPoint
                    obj.modifyAccessPoint(obj.EditIndex, 'Position', x, y)
                    obj.setTitle('Moving AP%d...', obj.EditIndex)
                case interactive.EditModes.Mobile
                    obj.modifyMobile(obj.EditIndex, 'Position', x, y)
                    obj.setTitle('Moving MOB%d...', obj.EditIndex)
                otherwise
                    assert(false, illegalstatemessage)
            end
            obj.EditMotion = true;
...
function u = unitdifference(entity1, entity2) %#ok<DEFNU>
u = unit([ ...
    entity2.PositionX - entity1.PositionX, ...
    entity2.PositionY - entity1.PositionY
    ]);
end
...
% -------------------------------------------------------------------------
function result = onofftological(s)
result = strcmpi(validatestring(s, {'on', 'off'}), 'on');
end
...
for i = 1 : size(xmobile, 1)
    for j = 1 : size(xmobile, 2)
        ...
        for accesspoint = accesspoints
            ...
            receivedpower(accesspoint.Index) = specfun.fromdb(powerdb);
        ...
        interferencepower = specfun.fromdb(options.MinimumDiscernableSignal); % "noise ..."
        for accesspoint = accesspoints
            if isinterferingpair(accesspoint, assignedaccesspoint)
        ...
        sinr(i, j) = specfun.todb(maxreceivedpower, interferencepower);
        
    end
end

    function gain = friisdb(distance)
        % Friis free-space attenuation in dB at distance
        gain = 2*specfun.todb(options.Wavelength, 4*pi*distance);
    end
...
isstructorempty = @(s) isstruct(s) || isempty(s);
assert(isstructorempty(walls))
assert(isstructorempty(accesspoints))
assert(isstructorempty(mobiles))
...
for accesspoint = accesspoints
    for mobile = mobiles
        ...
        pathgainsum = 2*specfun.todb(options.Wavelength, 4*pi*d);
...
for mobile = mobiles
    ...
    pdit = specfun.fromdb(options.MinimumDiscernableSignal);
    for accesspoint = accesspoints
        if isinterferingpair(accesspoint, assignedaccesspoint)
            pdit = pdit + specfun.fromdb(receivedpower( ...
                accesspoint.Index, mobile.Index));
        end
    end
    mobile.DownlinkINP = specfun.todb(pdit);
    mobile.DownlinkSINR = mobile.DownlinkReceivedPower - mobile.DownlinkINP;
    
    %direction = unitdifference(mobile, assignedaccesspoint);
...
% at the MAC layer.
for accesspoint = accesspoints
    % "TINP" = Total uplink (Interference + Noise) Power
    tinpwatts = specfun.fromdb(options.MinimumDiscernableSignal);
    for mobile = mobiles
        assignedaccesspoint = accesspoints(mobile.AccessPoint);
        if isinterferingpair(accesspoint, assignedaccesspoint)
            %direction = unitdifference(mobile, accesspoint);
            tinpwatts = tinpwatts + ...
                specfun.fromdb( ...
                mobile.TransmissionPower + ...
...
for ii = 1 : nr
    for jj = 1 : nc
        ...
        for kk = 1 : nrap
            ...
            rxpowerdb(ii, jj, kk) = specfun.todb(prx(kk)); %#ok<AGROW>
            % <<--- Added by Jon ---
...
compare(specfun.todb(rxpower), rxpowerdb)
intermediate = struct( ...
    'RxPower', rxpower, ...
    'Friis', friis, ...
...
% -------------------------------------------------------------------------
function tf = isround(a)
tf = isequal(a, fix(a));
end

% -------------------------------------------------------------------------
function map = binarycolormap
map = [
    graphics.rgb.salmon;
    graphics.rgb.lime;
    ];
...
% -------------------------------------------------------------------------
function mds = minimumdiscernablesignal
...
% -------------------------------------------------------------------------
function varargout = facevertextolines(faces, vertices)
%LINESTOFACEVERTEX Face-vertex representation from line segments.
% [X1,Y1,X2,Y2]=FACEVERTEXTOLINES(F,V) extracts pairs of line vertices
% from a face-vertex representation i.e. eliminating the face array F at
% the cost of duplication of the information in the vertex array V.
% See also LINESTOFACEVERTEX.

narginchk(2, 2)
assert(ismember(nargout, [0, 1, 2, 4]))
...
varargout = mat2cell( ...
    ...
    size(faces, 1), ...
    repmat(4/nout, 1, nout));
end

function structScalar = tabularvertcat(structArray)
structScalar = datatypes.struct.structfun( ...
    @vertcat, structArray, 'UniformOutput', false);
end

% -------------------------------------------------------------------------
function compare(actual, expected, label, tol)
narginchk(2, 4)
...
infnanmask = isinfnan(actual);
assert(all(isinfnan(expected(infnanmask))))
ninfmask = ~infnanmask;
diff = abs(actual(ninfmask) - expected(ninfmask));
%scale = abs(expected(ninfmask)) + eps(1.0); 
scale = abs(expected(ninfmask)) + tol;
[discrepancy, index] = max(diff(:)./scale(:));
if ~isempty(label)
    label = sprintf('(%s)', label);
end
if isempty(actual) || discrepancy < tol 
    if ~isempty(label)
        %fprintf('  discrepancy = %.1e: OK %s\n', discrepancy, label)
    end
else
    %cprintf('*red', ...
    error( ...
        'Max relative error: |(%g) - (%g)|/(%g) = (%g)/(%g) = %g exceeds %g %s\n', ...
        actual(index), expected(index), scale(index), ...
        diff(index), scale(index), ...
        discrepancy, ...
        tol, ...
        label)
end
...
% -------------------------------------------------------------------------
function mask = isinfnan(x)
mask = isnan(x) | isinf(x);
end
...
%SUPPRESS Context for suppressing output.
end
